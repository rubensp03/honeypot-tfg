...
def analizar_base_datos():
    ...
    for t in tablas['name']:
        if 'ataque' in t.lower() or 'log' in t.lower() or 'alerta' in t.lower():
            tabla_principal = t
            break
    ...
    if col_cve:
        print("\n=== TOP 10 CVEs ===")
        # Filtrar valores nulos, vacíos y "Desconocido" o "N/A"
        top_cves = pd.read_sql_query(f"SELECT {col_cve}, COUNT(*) as cantidad FROM {tabla_principal} WHERE {col_cve} IS NOT NULL AND {col_cve} != '' AND {col_cve} != 'Desconocido' AND {col_cve} != 'N/A' GROUP BY {col_cve} ORDER BY cantidad DESC LIMIT 10", conn)
        if not top_cves.empty:
            print(top_cves.to_string(index=False))
        else:
            print("No se encontraron registros con CVEs específicos.")
    else:
